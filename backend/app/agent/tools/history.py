...
def get_recent_meetings(
    ...
    """
    Retrieve a list of the user's recent meetings with ID, title,
    created date, and summary preview.
    """
    logger.info(
        f"Executing get_recent_meetings tool with limit={limit}"
    )
    ...
    try:
        ...
        if not meetings:
            logger.info(
                "No meetings found for current user."
            )
            return "No meetings found."

        result = (
            f"Found {len(meetings)} recent meeting(s):\n\n"
        )
        ...
        logger.exception(
            f"Error in get_recent_meetings tool: {str(e)}"
        )
